...
constexpr unsigned long DIST_OPTIMIZATION_PERIOD_US = 200000;
constexpr unsigned long DIST_BROADCAST_PERIOD_US = 100000;
constexpr float DIST_OUTPUT_BLEND = 0.10f;
constexpr float DIST_OUTPUT_MAX_STEP = 0.004f;
constexpr float DIST_OUTPUT_DEADBAND = 0.004f;
constexpr float OUTPUT_OFF_MARGIN_LUX = 1.0f;
...
static float moveDutyToward(float current, float target) {
    if (fabsf(target - current) <= DIST_OUTPUT_DEADBAND) {
        return clampUnitValue(current);
    }
...
static float activeLuxReference() {
    if (currentState == PID_STATE) {
        return setpointLux;
    }
    if (currentState == DIST_STATE) {
        return distributedController.getReference();
    }
    return 0.0f;
}

static float clampOutputWhenOverlit(float output_counts, float measured_lux) {
    const float reference_lux = activeLuxReference();
    if (reference_lux <= 0.0f) {
        return output_counts;
    }
    if (measured_lux < (reference_lux + OUTPUT_OFF_MARGIN_LUX)) {
        return output_counts;
    }

    if (currentState == PID_STATE) {
        controller.reset_integrator();
    } else if (currentState == DIST_STATE) {
        distributedTargetDuty = 0.0f;
        distributedAuxCache = 0.0f;
    }
    return 0.0f;
}
...
void handleResponse(CoreMail mail) {
    if (mail.sourceId == MY_NODE_ID) return; // ignore own broadcast echo    

    switch ((Command)mail.command) {
        case CMD_GET_LUX_MEASURE:
            calibration.trigger(CalibrationFSM::Trigger::LUX_RESPONSE, mail);
            break;

        case CMD_SET_DUTY:
            calibration.trigger(CalibrationFSM::Trigger::DUTY_ACK, mail);
            break;
...
void loop() {
    ...
    calibration.trigger(CalibrationFSM::Trigger::TICK);
    ...
    u = clampOutputWhenOverlit(u, lux);
    if (currentState == DIST_STATE && broadcast_state) {
        aux_to_broadcast = distributedAuxCache;
    }
